...
- July 16 retest with latched close tracking reported pickup success, but the robot drove too close; the block ended about 1 inch from the robot. Added a visible pickup handoff so the robot can stop at a low, centered, still-visible frame instead of waiting for the target to disappear.
...
- July 16 final-camera-angle retest stopped with the block still about 4 inches away. The approach was good, but the arm-only pickup transition did not move the chassis far enough for the claw to reach. The combined approach-and-pickup script now drives forward briefly while the arm lowers into pickup pose. Default calibration is `24%` for `0.25s`, tunable with `--pickup-drive-power` and `--pickup-drive-seconds`.
