...
- July 16 retest with latched close tracking reported pickup success, but the robot drove too close; the block ended about 1 inch from the robot. The useful stop frame was earlier: about `11cm`, `offset=-4`, `y=389`, and `S5=1230`. Added a visible pickup handoff so the robot can stop at that low, centered, still-visible frame instead of waiting for the target to disappear.
