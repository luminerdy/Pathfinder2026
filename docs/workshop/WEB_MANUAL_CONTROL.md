...
| Sonar checkbox | Shows the current sonar distance on the live camera feed |
...
- Sonar: the current forward sonar distance is shown while the Sonar checkbox is selected.

Manual drive commands are blocked while automation is running. Press `Stop Automation` before returning to manual driving.
Block Detect and Sonar are camera overlays only. They start turned off and do not drive the robot.
