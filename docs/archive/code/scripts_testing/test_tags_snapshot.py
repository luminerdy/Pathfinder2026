...
def test_tags_snapshot():
    ...
    filename = REPO_ROOT / f"tag_test_{int(time.time())}.jpg"
    ...
    if tags:
        ...
        annotated_file = filename.with_name(f"{filename.stem}_annotated.jpg")
        cv2.imwrite(str(annotated_file), frame)
        print()
        ...
        # Quality assessment
        areas = [
            (max(tag.corners[:, 0]) - min(tag.corners[:, 0])) *
        ...
        if max_area > 50000:
            print("  [OK] Excellent (very close or large tags)")
        elif max_area > 20000:
            print("  [OK] Good")
        elif max_area > 5000:
            print("  [!!] Fair")
        ...
        if max_area > 31000:
            ...
            print(f"  [OK] {improvement:.0f}% larger detection area!")
