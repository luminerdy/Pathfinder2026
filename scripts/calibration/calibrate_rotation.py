...
try:
    ...
    if initial_sonar:
        print(f"  Sonar: {initial_sonar / 10.0:.0f}cm")
    print()
        ...
        if actual_rotation is not None:
            ...
            if sonar_log:
                ...
                print(f"  Sonar: {len(sonar_log)} samples, "
                      f"{min_sonar / 10.0:.0f}-{max_sonar / 10.0:.0f}cm "
                      f"(avg {avg_sonar / 10.0:.0f}cm)")
    ...
    if not results:
        ...
        # Save to file
        results_file = REPO_ROOT / 'rotation_calibration.txt'
